...
function MockEnqForm({ isMobile,data,step, setStep, submitted,setSubmitted, submittedId,setSubmittedId, setTotalAmount, totalAmount, selectedBatch, setSelectedBatch, selectedChild, setSelectedChild }) {
  ...
  return (
    <>
      ...
      {step === 2 && (
        <>
          <ChildSelector isMobile={isMobile} title={data.mockTestTitle} setTotalAmount={setTotalAmount} setSelectedBatch={setSelectedBatch} selectedChild={selectedChild} setSelectedChild={setSelectedChild} setStep={setStep} />
        </>
      )}
      {(step === 3 ) && (
